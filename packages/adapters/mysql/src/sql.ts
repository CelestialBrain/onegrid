// =============================================================================
// MySQL SQL compiler — translates a BlockRequest into a parameterized
// SELECT against a MySQL table. Pure: no `mysql2` dependency, no I/O.
//
// Diverges from the Postgres compiler (@onegrid/postgres) at three
// places, which double as protocol-leak detectors per the v0.0.8
// critical-path rationale:
//
//   1. Identifier quoting: MySQL uses backticks (`); Postgres uses
//      double quotes ("). Same shape, different character — confined
//      to `quoteIdent`.
//   2. NULLS FIRST/LAST: MySQL has no native syntax. Emulated via
//      `ORDER BY col IS NULL DESC|ASC, col DIR` so nulls sort
//      consistently. Postgres has native `NULLS FIRST|LAST`.
//   3. Row-tuple comparison: MySQL supports `(a,b) > (1,2)` semantics
//      since 5.7; the predicate is identical. So this is NOT a
//      divergence — confirming the protocol shape is portable.
//
// Identifiers are backtick-quoted; values flow through `?`
// placeholders (mysql2 convention; positional, not numbered).
// =============================================================================
...
export interface MyTableDescriptor {
  /** Schema-qualified table identifier, e.g. `myapp.orders`. */
...
function compileGroupedQuery(
  ...
  const aggregationProjections = (req.aggregations ?? [])
    ...
    .join(', ');
  // Note: MySQL doesn't have a native ::int / ::float cast like
  // Postgres; CAST(... AS SIGNED) / CAST(... AS DOUBLE) does the
  // same. CAST is wrapped per fn so result types stay stable
  // across empty groups + matching the @onegrid/data aggregator's
  // behavior.
  const projection =
    [
      ...groupCols,
      'CAST(COUNT(*) AS SIGNED) AS `__count__`',
...
function compileAggregation(
  ...
  switch (agg.fn) {
    case 'sum':
      fnExpr = `CAST(COALESCE(SUM(${col}), 0) AS DOUBLE)`;
      break;
    case 'avg':
      fnExpr = `CAST(AVG(${col}) AS DOUBLE)`;
      break;
    case 'count':
      fnExpr = `CAST(COUNT(${col}) AS SIGNED)`;
      break;
    case 'countDistinct':
      fnExpr = `CAST(COUNT(DISTINCT ${col}) AS SIGNED)`;
      break;
    case 'min':
      fnExpr = `MIN(${col})`;
      break;
    case 'max':
      ...
      break;
    default:
      throw new Error(`@onegrid/mysql: unsupported aggregation fn "${agg.fn}".`);
  }
...
function compileComparison(
  ...
  // MySQL string comparisons honor the column's collation by default
  // (case-insensitive for `_ci` collations). When `caseSensitive`
  // is true, we wrap with `BINARY` to force byte comparison.
  const cs = node.caseSensitive !== false;
  const wrap = cs ? (s: string): string => `BINARY ${s}` : (s: string): string => s;
  switch (node.op) {
    case 'eq':
    case 'neq':
    case 'lt':
    ...
    case 'notContains': {
      ...
      return `${wrap(col)} ${not}LIKE ?`;
    }
    case 'startsWith': {
      params.push(`${escapeLike(String(node.value ?? ''))}%`);
      return `${wrap(col)} LIKE ?`;
    }
    case 'endsWith': {
      params.push(`%${escapeLike(String(node.value ?? ''))}`);
      return `${wrap(col)} LIKE ?`;
...
function compileOrderBy(
  ...
  // MySQL emulates NULLS FIRST/LAST via a leading IS NULL ordering
  // expression. `nulls = 'last'` → `col IS NULL ASC` (FALSE first,
  // TRUE last); `nulls = 'first'` → `col IS NULL DESC`.
  const fields: string[] = [];
  for (const field of sort) {
    requireColumn(field.columnId, table);
    const dir = field.direction === 'asc' ? 'ASC' : 'DESC';
    const nullsLast = (field.nulls ?? 'last') === 'last';
    fields.push(`${quoteIdent(field.columnId)} IS NULL ${nullsLast ? 'ASC' : 'DESC'}`);
    fields.push(`${quoteIdent(field.columnId)} ${dir}`);
  }
  // Tiebreaker on the primary key. Direction matches all-DESC sorts
  // or `direction: 'before'`, otherwise ASC. Mixed-direction keyset
  // is a v0.0.9 follow-up.
...
function escapeLike(input: string): string {
  // MySQL LIKE: `_`, `%`, and `\` must be escaped. The default escape
  // character is backslash; queries don't need an explicit ESCAPE
  // clause.
  return input.replace(/\\/g, '\\\\').replace(/%/g, '\\%').replace(/_/g, '\\_');
}

function requireColumn(columnId: string, table: MyTableDescriptor): void {
  if (columnId === table.primaryKey) return;
  if (!table.columns.includes(columnId)) {
    throw new Error(
      `@onegrid/mysql: unknown column "${columnId}" (not in table descriptor).`,
    );
  }
}
